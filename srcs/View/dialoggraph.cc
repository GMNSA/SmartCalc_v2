...
void DialogGraph::openGraphic() {
  ...
  if (m_isError || m_xMax <= m_xMin) {
    ...
    double x = m_xMin;
    double moveX = 0.02;
    double y = calclulateXCustom(m_strNum, x, m_scale, m_isError) - m_res;
    // double y = calclulateX(m_strNum, QString::number(m_xMin, 'f', 2),
    // m_isError).toDouble();
    double y1 = 0;

    for (int i = -m_width * 50; i <= m_width * 50; ++i, x += moveX) {
      y1 = calclulateXCustom(m_strNum, x + moveX, m_scale, m_isError) - m_res;
      if (y >= -m_height && y <= m_height && y1 >= -m_height &&
          y1 <= m_height) {
        if (isnan(y1) || y1 == 0) continue;
...
void DialogGraph::dataConversion() {
  setWidth((ui->graphicsView->width() - OFFSET_WIDTH) / 2);
  setHeigth((ui->graphicsView->height() - OFFSET_HEIGHT) / 2);
  QString r = QString("((%1) + (%2)) / 2").arg(m_xMax).arg(m_xMin);
  m_res = calclulateX(m_strNum, r, m_isError).toDouble();
  int count = 0;
  double res0 = 0;
  double res1 = 0;

  while (!isfinite(m_res) && count < 1e8) {
    if (count < 1e2)
      ++count;
    else if (count < 1e6)
      count *= 5;
    ...
    res0 = calclulateX(m_strNum, r, m_isError).toDouble();
    r = QString("((%1) + (%2)) / 2 - %3").arg(m_xMax).arg(m_xMin).arg(count);
    res1 = calclulateX(m_strNum, r, m_isError).toDouble();
    if (!isnormal((res0) && isnormal(res1))) res0 = 0;
    if (!isnormal(res1) && isnormal(res0)) res1 = 0;
    m_res = (res0 + res1) / 2;
  }

  if (!isfinite(m_res)) m_res = 0;
  if (!m_xMin && !m_xMax) {
...
QString DialogGraph::calclulateX(QString str_, QString x_, bool &isError_) {
  QString res;
  ...
  if (isError_) {
    ...
    res = m_calculator.calculate(str_);

    if (res == "error" || res == "na") isError_ = 1;
  }
